...
export class HistoryDB {
  ...
  constructor(dbPath: string = '.cobalt/history.db') {
  ...
  getStats(): { totalRuns: number; oldestRun: string | null; newestRun: string | null } {
    ...
    const oldestRun = this.db.prepare('SELECT timestamp FROM runs ORDER BY timestamp ASC LIMIT 1').get() as any
    const newestRun = this.db.prepare('SELECT timestamp FROM runs ORDER BY timestamp DESC LIMIT 1').get() as any
