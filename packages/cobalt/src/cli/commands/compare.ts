  ...
  async run({ args }) {
    try {
      ...
      for (const evaluator in runA.summary.scores) {
        ...
        console.log(
          `  ${evaluator}: ${baselineScore.toFixed(2)} → ${candidateScore.toFixed(2)} ` +
          color(`${arrow} ${diff > 0 ? '+' : ''}${diff.toFixed(3)} (${percentChange > 0 ? '+' : ''}${percentChange.toFixed(1)}%)`)
        )
      }
      ...
      const improvements = Object.keys(runA.summary.scores).filter(evaluator => {
        const diff = (runB.summary.scores[evaluator]?.avg || 0) - runA.summary.scores[evaluator].avg
        return diff > 0
      })
      ...
      if (improvements.length > 0) {
        console.log(pc.green(`✓ ${improvements.length} improvement(s): ${improvements.join(', ')}`))
      }
