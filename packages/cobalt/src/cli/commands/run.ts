...
export default defineCommand({
	...
	async run({ args }) {
		...
		try {
			...
			globalThis.__cobaltCLIResultCallback = (report: ExperimentReport) => {
				reports.push(report)
			}

			// Set up global overrides for experiment()
			if (args.ci && config.thresholds) {
				globalThis.__cobaltCIThresholds = config.thresholds
			}
			if (args.concurrency) {
				...
				globalThis.__cobaltConcurrencyOverride = concurrencyValue
			}
			if (args.filter) {
