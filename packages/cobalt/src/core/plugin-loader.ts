 ...
 */
export async function loadPlugin(path: string, cwd: string = process.cwd()): Promise<PluginDefinition> {
  ...
  try {
    ...
    console.log(`  Provides ${plugin.evaluators.length} evaluator(s): ${plugin.evaluators.map(e => e.type).join(', ')}`)
 ...
 */
export async function loadPlugins(paths: string[], cwd: string = process.cwd()): Promise<PluginDefinition[]> {
