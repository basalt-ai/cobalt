...
describe('loadResult', () => {
	...
	it('should throw when run ID not found', async () => {
		...
		vi.mocked(existsSync).mockReturnValue(true)
		vi.mocked(readdir).mockResolvedValue(['other_file.json'] as unknown)
...
describe('listResults', () => {
	...
	it('should list all JSON files sorted newest first', async () => {
		...
		const { readdir, readFile } = await import('node:fs/promises')
		vi.mocked(existsSync).mockReturnValue(true)
		vi.mocked(readdir).mockResolvedValue(['old_test_aaa.json', 'new_test_bbb.json'] as unknown)
	...
	it('should filter by experiment name', async () => {
		...
		const { readdir, readFile } = await import('node:fs/promises')
		vi.mocked(existsSync).mockReturnValue(true)
		vi.mocked(readdir).mockResolvedValue(['a.json', 'b.json'] as unknown)
	...
	it('should skip malformed JSON files', async () => {
		...
		const { readdir, readFile } = await import('node:fs/promises')
		vi.mocked(existsSync).mockReturnValue(true)
		vi.mocked(readdir).mockResolvedValue(['bad.json', 'good.json'] as unknown)
	...
	it('should calculate avgScores from summary', async () => {
		...
		const { readdir, readFile } = await import('node:fs/promises')
		vi.mocked(existsSync).mockReturnValue(true)
		vi.mocked(readdir).mockResolvedValue(['a.json'] as unknown)
