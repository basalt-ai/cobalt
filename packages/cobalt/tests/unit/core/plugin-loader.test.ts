...
import { loadPlugin, loadPlugins, registerPluginEvaluators } from '../../../src/core/plugin-loader.js'
import { registry as globalRegistry } from '../../../src/core/EvaluatorRegistry.js'
import type { PluginDefinition, EvaluatorPlugin } from '../../../src/core/plugin.js'
...
describe('Plugin Loader', () => {
  ...
  describe('Integration Tests', () => {
    ...
    it('should allow plugins to access config and apiKey parameters', async () => {
      ...
      const resultWithConfig = await handler(
        { threshold: 0.9 },
        { item: {}, output: '' }
      )
      expect(resultWithConfig.score).toBe(0.9)

      const resultWithAuth = await handler(
        {},
        { item: {}, output: '' },
        'test-api-key'
      )
